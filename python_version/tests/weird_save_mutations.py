...
def reduce_QP(QP, warnings=True):
    ...
    for e in edges_to_remove:
        ...
        if in_a_loop:
            ...
            print("edge %d is in a loop:"%e+str(tuple(sorted(loop))))
    if len(loops) > 0:
        ...
        if len(terms_for_e) > 0:
            ...
            while not found_replacement and ctr < len(edges_to_remove):
                ...
                for (term, coef) in terms_for_e:
                    ...
                    problem_terms = [x for x in term if x in edges_to_remove]
                    if len(problem_terms) > 0:
                        found_replacement = False

                        if any([x in met_terms for x in problem_terms]):
                            ctr = len(edges_to_remove)
                            #print("here we are...")
                            break

                        met_terms.update(problem_terms)
    ...
    print("need to save: "+str(quad_terms_to_save))
    edges_to_remove = [x for x in edges_to_remove if x not in quad_terms_to_save]
